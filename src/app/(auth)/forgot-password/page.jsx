...
export default function Register() {
  ...
  const onSubmit = (data) => {
    ...
    try {
      const response = AuthService.getEmailForgotPassword(data);
    ...
    <div class="h-full">
      <div class="relative flex  from-transparent to-white/80 min-h-[35rem] md:min-h-[95vh] [@media(min-width:2300px)]:min-h-[90vh] flex-col lg:flex-row items-center justify-center lg:gap-[2rem] bg-[url('/Home.png')] bg-cover bg-opacity-10  mt-[3.5rem] py-5 md:py-0 ">
        ...
        {!showNotif ? (
          <div class="mx-1 bg-white/95 backdrop-filter backdrop-blur-md mb-12 lg:mb-0 p-6 shadow-lg flex flex-col justify-center gap-4  w-full max-w-[29rem] rounded-sm">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="flex flex-col gap-4">
                <div class="relative w-full min-w-[200px] drop-shadow-sn h-10">
                  <input
          ...
          <div class="mx-1 bg-white/95 backdrop-filter backdrop-blur-md mb-12 lg:mb-0 p-6 shadow-lg flex flex-col justify-center items-center gap-1 w-full max-w-[29rem] rounded-sm">
            <Typography variant="lead" className="text-justify">
              If an account with the email address you provided exists in our
              system, you will receive an email with instructions on how to
              reset your password. Please check your inbox and follow the steps
              to regain access to your account.
            </Typography>
            <Link href={"/login"} onClick={() => setShowNotif(false)}>
